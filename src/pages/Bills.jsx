...
export default function Bills() {
    ...
    mutationFn: async (id) => {
      ...
      if (billToDelete) {
        ...
        (billToDelete.line_items || [])
          ...
          .forEach(item => {
            if (!itemQuantityMap[item.item_id]) itemQuantityMap[item.item_id] = 0;
        ...
        if (Object.keys(itemQuantityMap).length > 0) queryClient.invalidateQueries({ queryKey: ['items'] });
      }
  ...
  const handleRefresh = async () => await queryClient.invalidateQueries({ queryKey: ['bills'] });
              ...
              <div className="flex flex-row gap-2 w-full sm:w-auto">
                <div className="flex-1 sm:w-40">
                  ...
                  <div className="relative">
                    ...
                    />
                    {dateFrom && (
                      <button type="button" onClick={() => setDateFrom('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400"><X className="h-4 w-4" /></button>
                    )}
                  </div>
                </div>
                <div className="flex-1 sm:w-40">
                  ...
                  <div className="relative">
                    ...
                    />
                    {dateTo && (
                      <button type="button" onClick={() => setDateTo('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400"><X className="h-4 w-4" /></button>
                    )}
                  </div>
                </div>
              </div>
