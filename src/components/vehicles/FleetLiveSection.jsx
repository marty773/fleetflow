...
function VehicleRow({ motiveVehicle, localVehicle, isSelected, isExpanded, onClick, onFaultCodesClick }) {
  const {
  ...
  const isMoving = isActive && speed != null && speed > 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 py-3 px-4">
        {/* Status indicator: spinning arrow when moving, dot when idle */}
        <div className="flex items-center flex-shrink-0">
          {isMoving ? (
            <div
              className="w-5 h-5 flex items-center justify-center rounded-full bg-green-500 shadow-[0_0_0_3px_rgba(34,197,94,0.2)]"
        ...
        <div className="hidden sm:flex items-center justify-center flex-shrink-0" style={{width:'40px'}}>
          {fault_codes.length > 0 ? (
            <button
              onClick={e => { e.stopPropagation(); onFaultCodesClick && onFaultCodesClick(fault_codes, name); }}
              className="flex items-center gap-1"
            >
              <Badge variant="destructive" className="text-xs gap-1 cursor-pointer hover:bg-red-700">
...
function FaultCodesDialog({ vehicleName, faultCodes, onClose }) {
  return (
    <Dialog open={!!faultCodes} onOpenChange={onClose}>
            ...
            <div key={i} className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg p-3">
              <div className="flex items-start justify-between gap-2">
                <div className="flex-1">
                  <p className="text-sm font-semibold text-red-700 dark:text-red-400 font-mono">
                    {fc.code || fc.spn || (typeof fc === 'string' ? fc : 'Code')}
                    {fc.fmi != null ? ` / FMI ${fc.fmi}` : ''}
                  </p>
                  {fc.description && (
                    <p className="text-xs text-red-600 dark:text-red-300 mt-0.5">{fc.description}</p>
                  )}
                  {fc.source && (
                    <p className="text-xs text-slate-500 mt-0.5">Source: {fc.source}</p>
                  )}
                  {fc.updated_at && (
                    <p className="text-xs text-slate-400 mt-0.5">{new Date(fc.updated_at).toLocaleString()}</p>
                  )}
                </div>
                {fc.status && (
                  <span className="text-xs bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400 px-1.5 py-0.5 rounded capitalize shrink-0">{fc.status}</span>
                )}
              </div>
            </div>
          ))}
...
export default function FleetLiveSection({ vehicles = [] }) {
  ...
  const [faultDialog, setFaultDialog] = useState(null); // { codes, vehicleName }
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 mb-6 overflow-hidden">
      {faultDialog && (
        <FaultCodesDialog
          vehicleName={faultDialog.vehicleName}
          faultCodes={faultDialog.codes}
          onClose={() => setFaultDialog(null)}
        />
      )}
      {/* Header */}
            ...
            <>
              ...
              {filtered.map(mv => (
                <VehicleRow
                  ...
                  onFaultCodesClick={(codes, name) => setFaultDialog({ codes, vehicleName: name })}
                />
              ))}
            </>
          )}
